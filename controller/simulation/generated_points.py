...
def generate_lane_edges(centerline, lane_width=1):
    left_edge = []
    right_edge = []
...
def simulate_camera_detection(
    vehicle_pos, left_lane, right_lane, view_distance=20, noise_std=0.1
):
    x, y = vehicle_pos[:2]
...
def run_simulation(output_path):
    ...
    for i in range(steps):
        ...
        controls.append([mpc.normalize(accel, mpc.max_deceleration, mpc.max_acceleration), mpc.normalize(steering, -mpc.max_steering_angle, mpc.max_steering_angle)])
