...
class MultiTask():
    def __init__(self, model_path, confThreshold=0.5):
        self.classes = list(map(lambda x: x.strip(), open('lane-detector-yolo/models/classes', 'r').readlines()))
        print(self.classes)
        ...
        self.session = ort.InferenceSession(model_path, session_options=so, providers=providers)
