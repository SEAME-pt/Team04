...
class INA219 {
   ...
   private:
    ...
    auto readRegister(uint8_t register_value) -> uint16_t;
    void writeRegister(uint8_t register_address, uint16_t register_value);
    auto determineCurrentLsb(float max_expected_amps, float max_possible_amps) -> float;
    ...
    */
     ...
     */
    auto supplyVoltage() -> float;

    /**
     * @brief Reads the current register from the sensor and converts to milliamps.
     *
     * @return float Current in milliamps (mA) going through the sensor. "Inf" or "NaN" when
     ...
     *
     * @return float Bus power consumption in milliwatts (mW).  "Inf" or "NaN" when overflow is
     * detected.
     */
    auto power() -> float;
    ...
    float __GAIN_VOLTS[4] = {0.04F, 0.08F, 0.16F, 0.32F};
